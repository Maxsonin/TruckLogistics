...
from app.core.driver.dto.driver import UpdateDriverDTO
...
class EditDriverDialog(QDialog):
    def __init__(self, driver: Driver, parent: QWidget | None = None) -> None:
        ...
        self._name = QLineEdit(driver.name)
        self._name.setObjectName("formField")
        form.addRow("Name *", self._name)

        self._phone = QLineEdit(driver.phone or "")
    ...
    def _on_submit(self) -> None:
        ...
        try:
            with get_session() as session:
                DriverService(DriverRepository(session)).update_driver(
                    self._driver.id, UpdateDriverDTO(name=name, phone=phone)
                )
