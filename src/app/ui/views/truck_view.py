...
from app.ui.dialogs.add_truck_dialog import AddTruckDialog
from app.ui.dialogs.truck_detail_dialog import TruckDetailDialog
from app.ui.theme import colors

_HEADERS = ["ID", "Plate Number", "Model"]
...
class TruckView(QWidget):
    def __init__(self) -> None:
        ...
        self._detail_dialog: TruckDetailDialog | None = None
    ...
    def _on_add(self) -> None:
        ...
        if dialog.exec() == AddTruckDialog.DialogCode.Accepted:
            self.refresh()
    ...
    def _on_row_clicked(self, row: int, _col: int) -> None:
        ...
        if item is None:
            return
        truck_id: str = item.data(Qt.ItemDataRole.UserRole)
        ...
        self._detail_dialog = TruckDetailDialog(truck, parent=self)
        self._detail_dialog.finished.connect(lambda _: setattr(self, "_detail_dialog", None))
        self._detail_dialog.truck_updated.connect(self.refresh)
        self._detail_dialog.truck_deleted.connect(self.refresh)
    ...
    def refresh(self) -> None:
        ...
        for truck in self._trucks:
            ...
            for col, value in enumerate([truck.id[:8], truck.plate_number, truck.model or "—"]):
                item = QTableWidgetItem(value)
                item.setData(Qt.ItemDataRole.UserRole, truck.id)
                if col == 0:
                    item.setToolTip(truck.id)
